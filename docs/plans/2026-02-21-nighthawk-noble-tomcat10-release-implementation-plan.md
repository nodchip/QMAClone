...
**Files:**
- Modify: `pom.xml`
- Modify: `deploy_qmaclone_tomcat10.ps1`
...
```
- `deploy_qmaclone_tomcat10.ps1` は既定を開発機向け `GwtDraftCompile=$true` のまま維持し、本番配備時のみ `-GwtDraftCompile:$false` を明示して非ドラフト化する。
...
```bash
git add pom.xml deploy_qmaclone_tomcat10.ps1 ops/scripts/deploy/deploy_qmaclone_tomcat10.sh
...
**Files:**
- Modify: `deploy_qmaclone_tomcat10.ps1`（必要に応じて Tomcat10 互換化）
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\deploy_qmaclone_tomcat10.ps1 -ServiceName "Tomcat10" -TomcatBase "C:\Program Files\Apache Software Foundation\Tomcat 10.1" -GwtDraftCompile:$true
...
```bash
git add deploy_qmaclone_tomcat10.ps1 ops/log/2026-02-21-local-tomcat10-validation.log

...
2. theme 4テスト実行（初回）
   - コマンド: `mvn --% -Dsurefire.skip=false -DfailIfNoTests=false -Dtest=tv.dyndns.kishibe.qmaclone.client.setting.theme.ThemeCellTest,tv.dyndns.kishibe.qmaclone.client.setting.theme.ThemeProviderTest,tv.dyndns.kishibe.qmaclone.client.setting.theme.ThemeQueryCellTest,tv.dyndns.kishibe.qmaclone.client.setting.theme.ThemeQueryProviderTest test`
   - 結果: `BUILD FAILURE`（`ThemeProviderTest` と `ThemeQueryProviderTest` で Mockito/ByteBuddy が Java 25 を未サポート）

3. theme 4テスト実行（Java 25 回避設定あり）
   - コマンド: `mvn --% -Dnet.bytebuddy.experimental=true -Dsurefire.skip=false -DfailIfNoTests=false -Dtest=tv.dyndns.kishibe.qmaclone.client.setting.theme.ThemeCellTest,tv.dyndns.kishibe.qmaclone.client.setting.theme.ThemeProviderTest,tv.dyndns.kishibe.qmaclone.client.setting.theme.ThemeQueryCellTest,tv.dyndns.kishibe.qmaclone.client.setting.theme.ThemeQueryProviderTest test`
   - 結果: `Tests run: 6, Failures: 0, Errors: 0, Skipped: 0` / `BUILD SUCCESS`

4. 補足
   - 本検証ではテストコード変更なし。
   - Java 25 環境では、Mockito inline mock 利用テストに対して `-Dnet.bytebuddy.experimental=true` が必要。

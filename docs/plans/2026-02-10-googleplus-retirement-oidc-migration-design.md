...
## 10. 依存関係整理（gwt-plus削除）
- 本移行に合わせて `gwt-plus` を依存関係から削除対象に含める。
- 事前に `gwt-plus` 利用箇所を全件調査し、代替実装または不要化を確定する。
- 削除は以下の順で実施する。
1. `pom.xml` から `gwt-plus` 依存を除去する。
2. コンパイルエラー箇所を代替実装へ置換する。
3. GWT再コンパイルを実行し、配備成果物が最新であることを確認する。
4. build -> test を直列実行し、結果を記録する。
- 影響評価として、実行時クラスパスと生成成果物（`cache.js` 含む）への影響を明示する。

## 11. 設定画面UXフロー（2026-02-10追補）
### 11.1 ボタン文言（確定）
- `現在のユーザーコードをGoogleアカウントに紐づける`
- `Googleログインして紐づけ済みユーザーコードを表示する`
- `選択したユーザーコードに切り替える`
- `選択したユーザーコードとGoogleアカウントの紐づけを解除する`
...
- 連携済み初期状態:
  - `紐づける` は非表示。
  - `表示する` は表示。
  - 一覧取得後、件数に応じて `切り替える` と `解除する` を表示。
- 実行中状態:
  - 押下した操作の二重実行を防ぐため、表示中の操作ボタンは無効化する。
- 解除成功状態:
  - 全ボタンを無効化し、再読込を要求する。
  - 画面メッセージとポップアップの両方で `F5ボタンを押してページをリロードしてください。` を表示する。

### 11.3 件数別表示ルール
- 0件:
  - 一覧は空表示。
  - `紐づけ済みユーザーコードはありません` を画面表示。
- 1件:
  - 一覧と `解除する` を表示。
  - `切り替える` は非表示。
- 2件以上:
  - 一覧と `切り替える` と `解除する` を表示。

### 11.4 キャンセルと失敗
- Googleログインキャンセル:
  - 失敗扱いにせず、`Google連携はキャンセルされました` を画面表示。
  - 押下前の表示状態へ戻す。
- 通信失敗・保存失敗:
  - 該当操作ボタンを再有効化。
  - `処理に失敗しました。再試行してください。` を画面表示。

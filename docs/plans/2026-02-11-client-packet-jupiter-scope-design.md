...
2. Jupiter 対象（packet）
   - コマンド: `mvn --% -Dsurefire.skip=false -DfailIfNoTests=false -Dtest=tv.dyndns.kishibe.qmaclone.client.packet.PacketProblemTest,tv.dyndns.kishibe.qmaclone.client.packet.PacketRoomKeyTest test`
   - 結果: `Tests run: 27, Failures: 0, Errors: 0, Skipped: 0` / `BUILD SUCCESS`

3. 除外対象確認
   - コマンド: `rg -n "extends QMACloneGWTTestCaseBase" src/test/java/tv/dyndns/kishibe/qmaclone/client/packet/PacketPlayerSummaryTest.java`
   - 結果: `PacketPlayerSummaryTest` が `QMACloneGWTTestCaseBase` 継承であることを確認

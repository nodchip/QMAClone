...
- ミス: DevMode と Tomcat の差を考慮せず、クラスローダ起因の `NoClassDefFoundError` を見落とした。
- 改善: クラスローダ差を前提に、Jetty 関連変更時は `NoClassDefFoundError` と `ClassCastException` をセットで確認する。
- ミス: GWT 再コンパイル失敗後に古い `cache.js` が配信され、画面不整合が発生した。
- 改善: クライアント変更の完了条件に GWT 再コンパイル成功を必須化し、配備先が最新成果物を参照していることを確認する。
- ミス: WebSocket URL を環境固定値で扱い、接続先不一致を起こした。
- 改善: `protocol/host/contextPath` から URL を導出し、クライアントログに接続先 URL を必ず出力する。
- ミス: WebSocket 障害をサーバーログ確認前にクライアント側だけで断定した。
